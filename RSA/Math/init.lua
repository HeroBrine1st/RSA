...
local function mulNum(op1, op2)
    -- костыль // применяем правила умножения для обработки целых чисел поверх натуральных
    local inv1 = op1.inv
    local inv2 = op2.inv
    local res = op1 * op2
    res.inv = inv1 ~= inv2
...
local function subNum(op1, op2)
    -- костыль // применяем правила вычитания для обработки целых чисел поверх натуральных
    local inv1 = op1.inv
    local inv2 = op2.inv
...
local function addNum(op1, op2)
    --костыль // применяем a + b = a - (-b) для обработки целых чисел поверх натуральных
    local op2b = Long(op2)
...
local function modular_inversion(a, m)
    local x, _, d = extendedEuclideanAlgorithm(a, m)
    if d == longOne then
        local m2 = Long(m)
        m2.inv = true
...
local function Prime(L, debug)
    -- поиск простого числа среди множества чисел длиной L
    local prime
    repeat
        prime = RandomNum(L)
